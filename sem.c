...
Ranges* ranges_reflow(Range range, I16 bits) {
  Ranges* result = sem_ranges_init(2);
  I64 lo_converted = bits_convert(range.lo, bits);
  I64 hi_converted = bits_convert(range.hi, bits);
  if (lo_converted == range.lo && hi_converted == range.hi) {
    result->length = 1;
    result->pairs[0] = range;
  }
  else if ((lo_converted < range.lo && hi_converted == range.hi && range.hi < lo_converted-1)
       ||  (hi_converted > range.hi && lo_converted == range.lo && range.lo > hi_converted+1)) {
    // -128..118 \ 120..127
    result->length = 2;
    result->pairs[0].lo = bits_min(bits);
    result->pairs[0].hi = hi_converted;
    result->pairs[1].lo = lo_converted;
    result->pairs[1].hi = bits_max(bits);
  }
  else {
    // -128..127 or -128..119 \ 120..127
    result->length = 1;
    result->pairs[0].lo = bits_min(bits);
...
B8 type_is_subtype_rec(Block* block, Type* one, Type* two, Subtype_Visited* visited) {
  ...
  if (one->kind == two->kind) {
    ...
    case Type_Kind_int: {
      result = ranges_is_subrange(one->ranges, two->ranges);
    } break;
    case Type_Kind_ptr: {
...
Type* type_fun(Function* function) {
  ...
  for (;;) {
    i &= sem.ranges_set.cap - 1;
    Function* key = sem.function_set.keys[i];
    if (!key) {
      sem.function_set.keys[i] = function;
      break;
    }
    else {
...
Type* type_pointer(Pointer* pointer) {
  ...
  I32 i = 0;
  for (;;) {
    i &= sem.pointer_set.cap - 1;
    Pointer* key = sem.pointer_set.keys[i];
    if (!key) {
      sem.pointer_set.keys[i] = pointer;
      break;
    }
    else {
    ...
    case Type_Kind_record: {
      ...
      for (I32 i = 0; i < length; i++) {
        ...
        if (type_is_same(block, field.type, field_var.type)) {
          ...
          new_record->types[i] = type_auto_cast(block, 0, field.type, field_var.type);
        }
      }
...
void sem_ir(Block* block, Ir* ir) {
  ...
  case Ir_Kind_str: {
    ...
    {
      Record* str_record = type_record_init(ir->str->length);
      str_record->is_array = true;
      for (I32 i = 0; i < ir->str->length; i++) {
        Type* int_type = type_int(ir->str->base[i]);
        str_record->types[i] = type_define_size(8, int_type);
      }
      str_record_type = type_record(str_record);
    }
    Type* fun_type;
    {
      ...
      *records = hash_set_init(sem.perm_arena, 1);
      ...
      Record* len_record = type_record_init(1);
      ...
      function->arg = str_record_type;
      function->ret = type_compose(sem.type_none, ptr_type, records, 0);
      function->fun = sem.str_from_i8_array;
      fun_type = type_fun(function);
    }
    Hash_Set* records = arena_push(sem.perm_arena, sizeof(Hash_Set));
    *records = hash_set_init(sem.perm_arena, 1);
    hash_set_put(records, str_record_type);
    Types* auto_casts;
    fa_init(sem.perm_arena, auto_casts, 1);
    fa_add(auto_casts, fun_type);
    result = type_compose(sem.type_none, sem.type_none, records, auto_casts);
  } break;
    ...
    if (one_type->kind == Type_Kind_int) {
      if (type_is_const(one_type)) {
        ...
        *records = hash_set_init(sem.perm_arena, 2);
        Type* ptr_type = type_pointer_to(two_type);
        { // (len:one_type->int)
        ...
        { // (two_type; two_type; ... ; two_type)
          I64 length = ranges_min(one_type->ranges);
          Record* record = type_record_init(length);
          record->is_array = true;
          for (I32 i = 0; i < length; i++) {
            record->types[i] = two_type;
          }
          Type* record_type = type_record(record);
          hash_set_put(records, record_type);
        }
        result = type_compose(sem.type_none, ptr_type, records, 0);
      }
      else {
        assert(0);
      }
  ...
  case Ir_Kind_call: {
    ...
    if (fun_type == sem.bits_fun) {
      ...
      else if (arg_type->kind == Type_Kind_compose && arg_type->compose->records->len == 1) {
        Type* type = arg_type->compose->records->list[0];
        result = type_int(type->record->length);
      }
      else {
        assert(0);
      }
